...
#====================================================================#
#  ph.py                                                          #
#    Nexus interface to the Phonon simulation code.                   #
#                                                                    #
#  Content summary:                                                  #
#    Ph                                                           #
#      Simulation class for Phonon.                                   #
#                                                                    #
#    generate_ph                                                  #
#      User-facing function to create Ph simulation objects.      #
#                                                                    #
#====================================================================#
...
from ph_input import PhInput,generate_ph_input
from ph_analyzer import PhAnalyzer


class Ph(Simulation):
    input_type = PhInput
    analyzer_type = PhAnalyzer
    generic_identifier = 'ph'
    application = 'pw.x'
    ...
    def __init__(self,**sim_args):
        ...
        if group_atoms and isinstance(self.system,PhysicalSystem):
            self.warn('requested grouping by atomic species, but ph does not group atoms anymore!')
...
def generate_ph(**kwargs):
    ...
    if not 'input' in sim_args:
        ...
        sim_args.input = generate_ph_input(input_type,**inp_args)
    #end if
    ph = Ph(**sim_args)
